...
export type BulkLinkSyncCallbacks = {
  onEvent(event: BulkStreamEvent): void
}

export async function runBulkLinkSync(
  collections: string[],
  callbacks: BulkLinkSyncCallbacks,
): Promise<void> {
  const response = await fetch('/api/ai-translate/links/bulk', {
  ...
  if (!response.ok) {
    ...
    throw new Error(text || 'Bulk link synchronization request failed.')
  }
